...
<template>
  <view class="gift_layer flex_center">
    ...
    <view class="fail" wx:if="{{stepData.status == -2}}">
      <!-- <view class="close_yinju iconfont icon-guanbi" @tap="close"></view> -->
    ...
    <view class="fail" wx:if="{{stepData.status == 2}}">
      ...
      <view class="resulte_text">审核成功</view>
      <view class="notes">离奖品又近了一步</view>
    </view>
...
<script>
  ...
  @connect({
    stepData(state) {
      // this.status = state.drawTask.data.status = 2
